...
public interface DonarServiceRepo extends JpaRepository<Donar, Integer>{
	...
	@Query(nativeQuery = true, value = "update donar set age=:age, blood_group=:bloodGroup, donar_location=:donarLocation, donar_name=:donarName, email_id=:emailId, phone_number=:phoneNumber where donar_id=:donarId")
	@Modifying
	@Transactional
	public int update(@Param("donarName") String donarName, @Param("age") int age, @Param("bloodGroup") String bloodGroup, @Param("donarLocation") String donarLocation, @Param("emailId") String emailId,
			@Param("phoneNumber") String phoneNumber, @Param("donarId") int donarId);
